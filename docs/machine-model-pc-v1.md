...
- 8259A dual PIC: port-wired on `MachineBus`; ICW1–ICW4, OCW1 IMR, OCW2 non-specific/specific EOI, OCW3 IRR/ISR read select, OCW3 poll command, edge IR assert, fully nested priority, cascade on IR2. **Not yet:** Auto-EOI, rotate, special mask, special fully-nested mode, level-triggered runtime.
- 8259A OCW3 poll command (`P=1`): writing OCW3 with `P=1` to a command port (`0x20` master / `0xA0` slave) arms that chip; the **next read of that command port** is an interrupt-acknowledge cycle instead of an IRR/ISR read. It resolves the highest-priority unmasked request (fully nested, IMR applied), sets that IS bit, clears the IR bit (edge model), and returns `0x80 | level` (bit7 = interrupt pending, bits 2:0 = binary level). The poll is one-shot: later command-port reads return the `RR`/`RIS`-selected register again, and `P=1` leaves that selection untouched. Cascaded polling is software sequenced — poll the master (returns cascade level 2 on the PC AT and sets master IS2), then poll the slave command port for the slave level. With nothing pending the byte is `0x00`; bits 2:0 are unspecified by the datasheet in that case, so **zero is a documented model choice**. Reset / ICW1 state: poll disarmed (a fresh OCW3 `P=1` is required after every ICW1 sequence).
...
- 8259A: Intel 8259A Programmable Interrupt Controller datasheet (ICW1–ICW4, OCW1–OCW3 EOI/IMR/IRR/ISR, OCW3 format + Poll Command); classic PC cascade on IRQ2.
