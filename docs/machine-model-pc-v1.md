...
- Hardware IRQ path: `MachineBus::poll_external_irq` syncs PIT ch0 OUT → PIC IRQ0 then `DualPic::poll_irq` (INTA-style acknowledge + vector). CPU still delivers via `pending_irq` when `IF=1`.
