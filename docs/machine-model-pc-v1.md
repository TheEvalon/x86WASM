...
- FDC: Intel 82077AA + OSDev FDC ? `Fdc82077` on `0x3F0`?`0x3F5`/`0x3F7` (not `0x3F6`); DOR/MSR/FIFO/DIR/CCR accept; MSR RQM when DOR nRESET set; Specify (`0x03`) ? two parameter bytes (SRT|HUT, HLT|ND) stored, no result/IRQ; Recalibrate (`0x07`) ? one unit-select param, `pcn=0`, latches ST0 Seek End (`0x20|US`) and asserts IRQ; Sense Interrupt Status (`0x08`) ? result ST0+PCN with MSR RQM|DIO and IRQ clear (uses Recalibrate ST0 latch when present, else post-reset/`assert_irq6` stub `0xC0|DOR[1:0]`); `assert_irq6`/`clear_irq6` ? `irq_line` (DOR nRESET?DMA/IRQ bit3) ? `MachineBus::poll_external_irq` ? DualPic IRQ6 (vector `0x0E` with default ICW2 base `0x08`). **Not yet:** Seek/READ/WRITE/other commands, media image, DMA ch2 transfers, seek timing, disk-change timing.
