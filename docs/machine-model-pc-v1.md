...
- FDC: Intel 82077AA + OSDev FDC � `Fdc82077` on `0x3F0`�`0x3F5`/`0x3F7` (not `0x3F6`); DOR/MSR/FIFO/DIR/CCR accept; MSR RQM when DOR nRESET set; Sense Interrupt Status (`0x08`) ? result ST0+PCN with MSR RQM|DIO and IRQ clear; `assert_irq6`/`clear_irq6` ? `irq_line` (DOR nRESET?DMA/IRQ bit3) ? `MachineBus::poll_external_irq` ? DualPic IRQ6 (vector `0x0E` with default ICW2 base `0x08`). **Not yet:** other commands, media image, DMA ch2 transfers, automatic IRQ on media command completion, disk-change timing.
