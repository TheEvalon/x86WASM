...
- Hardware IRQ path: `MachineBus::poll_external_irq` syncs PIT ch0 OUT → IRQ0, 8042 OBF∧INT1 → IRQ1, CMOS IRQF → IRQ8, 8042 AUX OBF∧INT12 → IRQ12, primary IDE INTRQ∧¬nIEN → IRQ14, and secondary IDE → IRQ15 then `DualPic::poll_irq` (INTA-style acknowledge + vector). CPU still delivers via `pending_irq` when `IF=1`.
- 8259A dual PIC: port-wired on `MachineBus`; ICW1–ICW4 (incl. ICW4.AEOI Automatic EOI — ISR bit cleared at end of INTA / OCW3-poll acknowledge), OCW1 IMR, OCW2 non-specific/specific EOI, OCW3 IRR/ISR read select, OCW3 poll command, edge IR assert, fully nested priority, cascade on IR2. **Not yet:** rotate, special mask, special fully-nested mode, level-triggered runtime.
- 8259A OCW3 poll command (`P=1`): writing OCW3 with `P=1` to a command port (`0x20` master / `0xA0` slave) arms that chip; the **next read of that command port** is an interrupt-acknowledge cycle instead of an IRR/ISR read. It resolves the highest-priority unmasked request (fully nested, IMR applied), sets that IS bit, clears the IR bit (edge model), and returns `0x80 | level` (bit7 = interrupt pending, bits 2:0 = binary level). With ICW4.AEOI the IS bit is cleared again at the end of that acknowledge (same `ack_ir` path as hardware INTA). The poll is one-shot: later command-port reads return the `RR`/`RIS`-selected register again, and `P=1` leaves that selection untouched. Cascaded polling is software sequenced — poll the master (returns cascade level 2 on the PC AT and sets master IS2), then poll the slave command port for the slave level. With nothing pending the byte is `0x00`; bits 2:0 are unspecified by the datasheet in that case, so **zero is a documented model choice**. Reset / ICW1 state: poll disarmed (a fresh OCW3 `P=1` is required after every ICW1 sequence).
- 8254 PIT: port-wired on `MachineBus`; channel-0 programming + `ce`/OUT tick (`Pit8254::tick_ch0`, `Machine::tick_pit` also ticks ch2). Modes 0–5 move OUT; OUT rising edges drive IRQ0 (8259A master IR0). Mode 0 OUT rises at terminal count; modes 2/3 pulse OUT low one clock per period; modes 4/5 strobe OUT low for one clock at terminal count (one-shot: mode 4 re-armed by a new count, mode 5 by a GATE rising edge); mode 1 drives OUT low on the GATE trigger until terminal count (retriggerable). GATE low disables counting in modes 0/2/3/4 (forcing OUT high in modes 2/3) and has no effect in modes 1/5. Ch0/ch1 GATE assumed high, so GATE-triggered modes 1/5 only start counting on channel 2. Channel 2 GATE + speaker-data latch + OUT2 readback via port `0x61` bits 0/1/5 (no host audio). Guest wall-clock rate is **not** host-real-time (explicit tick quantum). **Not yet:** host speaker audio, ch1 DRAM refresh, mode 3 exact 50% duty, one-CLK count-load/trigger delay (loads take effect on the same model clock), BCD counting during tick, read-back command, port `0x61` NMI/parity/refresh bits.
...
- 8042 / PS/2: port-wired on `MachineBus` (`0x60`/`0x64`); self-test / config / enable-disable; OBF + config bit0 (INT1) → IRQ1 via `Machine::kbd_place_output` / `poll_external_irq`; make-code inject stub (`I8042::inject_scancode` / `Machine::kbd_inject_scancode`) → OBF when keyboard clock enabled (dropped when config bit4 set); config bit6 translation stored but **not** remapped (passthrough Set1 bytes); output-port `0xD0`/`0xD1` bit1 → `PhysMem` A20.
  - Second (auxiliary/mouse) **port**, controller side only: `0xA7` sets and `0xA8` clears config bit5 (aux clock disable); `0xA9` (test aux interface) answers `0x00` on the normal output buffer (OBF, **not** AUX OBF); `0xD4` routes the next `0x60` write to the aux device, recorded in `I8042::last_aux_device_write` / `aux_device_writes` (no device → no response byte). `I8042::inject_aux_byte` / `Machine::kbd_inject_aux_byte` fill the buffer with status bit0 (OBF) **and** bit5 (AUX OBF) set; reading `0x60` clears both. IRQ line selection follows the byte's source: keyboard data → IRQ1 (config bit0), aux data → IRQ12 (config bit1) via `I8042::irq12_line` / `poll_external_irq` (8259A slave IR4, vector `0x74` with default ICW2 base `0x70`); neither source can drive the other's line.
  - Status bits: bit0 OBF, bit1 IBF (instant completion; never sticky), bit2 system flag (mirrors config bit2), bit3 last write was command, bit5 AUX OBF (PS/2 meaning; the AT transmit/receive-timeout meaning is not modeled). Config bits: bit0 KBD interrupt enable, bit1 AUX interrupt enable, bit4 KBD clock disable, bit5 AUX clock disable, bit6 translation (stored only). Reset state: config `0x50` (keyboard clock disabled, aux clock **enabled**, both interrupt enables clear, translation on), output port `0xDF` (A20 on), output buffer empty, no recorded aux traffic.
  - **Not yet:** any PS/2 **mouse device** behind the aux port (`0xFF` reset / `0xFA` ACK / `0xF4` enable reporting / sample rate / resolution / 3-byte movement packets / wheel and 5-button extensions), aux clock-disable gating of host→device `0xD4` writes (it only gates `inject_aux_byte`), full AT keyboard protocol / Set2↔Set1 table, pulse-reset (`0xFE` / output-port bit0), interface test `0xAB`, diagnostic dump `0xAC`.
...
- 8259A: Intel 8259A Programmable Interrupt Controller datasheet (ICW1–ICW4 incl. AEOI, OCW1–OCW3 EOI/IMR/IRR/ISR, OCW3 format + Poll Command); classic PC cascade on IRQ2.
- 8254: Intel 8254 PIT datasheet — channel 0/2 control word, lo/hi access, latch, "Mode Definitions" modes 0/1/2/3/4/5 OUT + GATE-pin operations summary; ch0 OUT → IRQ0 via `Machine::tick_pit` / `poll_external_irq` level follow; ch2 GATE/OUT via port `0x61`. No host audio / DRAM-refresh / host-real-time claims.
...
- 8042: OSDev I8042 PS/2 Controller + IBM PC AT / PS/2 8042 programming model — port-wired; config INT1 + OBF → ISA IRQ1; make-code inject → OBF (clock-disable respected; translation passthrough); output-port bit1 → A20; second-port command set `0xA7`/`0xA8`/`0xA9`/`0xD4` + status bit5 AUX OBF + config bit1/bit5 → ISA IRQ12; see `devices::I8042` / `Machine::kbd`.
...
- VGA text: IBM VGA / OSDev Text UI / OSDev VGA Hardware / FreeVGA — `0xB8000` char+attr plane + CRTC `0x3D4`/`0x3D5` noop + Sequencer `0x3C4`/`0x3C5` noop (mode-03h defaults) + Misc Output `0x3C2`/`0x3CC` store/readback on `MachineBus`; **not** GC/ATC/Sequencer or Misc side effects/timing/render.
