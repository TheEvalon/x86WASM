...
| `0x71` | CMOS/RTC data |
| `0x60` | 8042 / PS/2 data — **no IRQ1** (this CMOS slice) |
...
- Hardware IRQ path: `MachineBus::poll_external_irq` syncs PIT ch0 OUT → IRQ0 and CMOS IRQF → IRQ8 then `DualPic::poll_irq` (INTA-style acknowledge + vector). CPU still delivers via `pending_irq` when `IF=1`.
