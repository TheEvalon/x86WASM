...
- 8042 / PS/2: port-wired on `MachineBus` (`0x60`/`0x64`); self-test / config / enable-disable; OBF + config bit0 (INT1) → IRQ1 via `Machine::kbd_place_output` / `poll_external_irq`; output-port `0xD0`/`0xD1` bit1 → `PhysMem` A20. **Not yet:** IRQ12/mouse, scancode device protocol, pulse-reset (`0xFE` / output-port bit0).
...
- 8042: OSDev I8042 PS/2 Controller + IBM PC AT 8042 programming model — port-wired; config INT1 + OBF → ISA IRQ1; output-port bit1 → A20; see `devices::I8042` / `Machine::kbd`.
