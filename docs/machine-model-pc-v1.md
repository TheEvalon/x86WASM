...
- CMOS/RTC: port-wired on `MachineBus`; status B PIE/AIE/UIE + status C PF/AF/UF/IRQF (read-to-clear); `CmosRtc::tick` / `Machine::tick_cmos` → IRQ8 (8259A slave IR0); Status A UIP + `CmosRtc::tick_second` / `Machine::tick_cmos_second` BCD sec→min→hour stub (SET inhibits); port `0x70` bit7 NMI mask stored (`CmosRtc::nmi_masked`) / `Machine::nmi_delivery_enabled`; `Machine::inject_nmi` latches CPU `#NMI` (IVT vector 2) when unmasked, drops when masked. Guest period/second quantum is **not** host-real-time. **Not yet:** host wall-clock/NTP sync, SMRAM/SMI / NMI nesting, exact crystal UIP pulse width, full calendar day/month/year.
