...
Everything here is **CPL 0 โ’ CPL 0 only**. Privilege switching, call gates,
TSS/task gates, LDT resolution, and paging are explicitly out of scope and
belong to later rounds.

## Authoritative sources

- Intel SDM Vol. 1 ยง3.5 (EIP), ยง3.6 + Table 3-4 (operand-size / address-size
  attributes), ยง6.5 (procedure calls, stack frames)
...
- Intel SDM Vol. 3 Chapter 5 (Protection): ยง5.3 limit checking, ยง5.8.1 direct
  far transfers
- Intel SDM Vol. 3 Chapter 6 (Interrupt and Exception Handling): ยง6.11
  (IDT descriptors), ยง6.12.1 (interrupt/trap gate delivery), ยง6.13 (error
  codes), ยง6.14 (exception/interrupt reference)

## Slice 1 โ€” `CS.D=1` default-32 execution
...
**Not supported by this slice.** 32-bit stacks (`SS.B=1`) โ€” pushes and pops
...
from the cached `SS.B` bit (SDM Vol. 3 ง3.4.5.1, Vol. 1 ง6.2.2). With `B=1`
...
to return `Unsupported` when prefixed with `0x67`. Per SDM Vol. 1 ง6.2.2 the
