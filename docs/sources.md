...
- PS/2 keyboard device protocol — OSDev Wiki "PS/2 Keyboard" (<https://wiki.osdev.org/PS/2_Keyboard>): host→device commands on data port `0x60` when the controller is not expecting a command parameter (contrast controller commands on `0x64` and pending write-config / write-output / write-aux); ACK `0xFA`; Reset `0xFF` → ACK then BAT `0xAA`; Get Keyboard ID `0xF2` → ACK then identification bytes (typically MF2 `0xAB` `0x83`); Enable/Disable Scanning `0xF4`/`0xF5` → ACK; responses present on keyboard OBF (not AUX OBF). This emulator implements that ACK subset in `I8042`; LEDs/typematic/scancode-set select remain unanswered
