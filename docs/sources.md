...
- PS/2 mouse device protocol — OSDev Wiki "PS/2 Mouse" (<https://wiki.osdev.org/PS/2_Mouse>) "Mouse Commands": host→device via KBC `0xD4`; Reset `0xFF` → ACK `0xFA`, BAT `0xAA`, device ID `0x00` (standard mouse); Get Device ID `0xF2` → ACK + ID; Enable/Disable Data Reporting `0xF4`/`0xF5` → ACK (reporting flag only in this stub; no sample-rate/resolution/packet stream)
