...
- Intel 82077AA CHMOS Single-Chip Floppy Disk Controller — DOR/MSR/FIFO/DIR/CCR; Specify (`0x03`) two params (SRT|HUT, HLT|ND), no result/IRQ; Sense Interrupt Status (`0x08`) ST0+PCN; DOR bit3 DMA/IRQ enable; IRQ on command complete
- OSDev Wiki Floppy Disk Controller — ports `0x3F0`–`0x3F7` excluding `0x3F6` (IDE); MSR RQM/DIO; Specify timing; Sense Interrupt clears IRQ; ISA IRQ6
