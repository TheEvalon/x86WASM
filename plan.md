...
Status (2026-08-10, round-13 cpu-vm86 lane `slice/r13-cpu-vm86` on base `f579e8c`): **in progress -- R13 cpu-vm86 lane.** VME `CLI`/`STI` → `VIF`; `PUSHF`/`POPF` method-4; soft-int redirect method-5/6 deepen; still **no** `CPUID.VME`. See `docs/cpu-r13-*.md`.
