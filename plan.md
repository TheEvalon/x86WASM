...
Status (2026-08-11, round-14 usb-timer lane `slice/r14-usb-timer` on base `9934398`): **in progress -- R14 usb-timer lane.** UHCI?PIC IRQ wire (PIIX3 PIRQD?classic IRQ11); UHCI IOC/USBSTS W1C + sticky USBINT deepen; LAPIC SVR sticky vector/enable/focus + soft-enable gating (EOI-suppress dropped); HPET legacy replacement explicit stub (`LEG_RT_CAP` clear, no PIT/RTC dual-fire). See `docs/uhci-r14-*.md`, `docs/lapic-r14-svr-stub.md`, `docs/hpet-r14-legacy-irq.md`. **Not** UHCI MSI; multi-APIC IPI; real LEG_RT Timer1?IRQ8; CPUID APIC stays clear.
