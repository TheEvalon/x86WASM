...
Status (2026-08-10, round-10 lane `slice/r10-cpu-vm86` on base `d832290`): **cpu-vm86 lane complete (4 slices).** VM86?CPL0 9-dword interrupt/exception frame; far JMP/CALL/RETF while `VM=1` (real-mode-like, stay in VM86); `INT n` IOPL `#GP` without VME (`INT3`/`INTO` not IOPL-sensitive). See `docs/cpu-r10-vm86-*.md`. **Not yet merged into a round-10 parallel tip.** Remaining honesty: VME/PVI, soft-int redirect bitmap, 16-bit IDT gates from VM86, call/task gates from VM86 still out.
