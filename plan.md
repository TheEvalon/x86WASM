...
- [ ] 32-bit paging — **partial, wired for a flat same-privilege kernel:** `crates/x86-mmu/src/paging/` plus round-4 interpreter wiring (`PagedBus`, `CR0.PG`, guest `CR2`/`CR3`/`CR4`, `#PF` delivery, restartability, page-crossing splits) and a persistent `Machine`-held TLB via `step_with_mmu`. CPUID advertises `PSE`/`PGE`/`CMOV` and family 6. The A/D-versus-fault deviation survived contact with real execution. **Not enough for a real OS:** no privilege-changing gates, no TSS stack switch, no `#DF`/triple-fault, no PAE/long mode. See `docs/cpu-r4-paging-integration.md`.
...
- [ ] ATAPI — **partial: detection + three packet commands.** A configured PACKET device remains detectable (signature, IDENTIFY PACKET DEVICE) and now runs `PACKET` (`A0h`) for exactly TEST UNIT READY, REQUEST SENSE, and INQUIRY (SFF-8020i); DEVICE RESET (`08h`) is implemented for PACKET devices. **Peripheral type remains `1Fh`, not `05h` CD-ROM.** ATA disks still abort `PACKET`/`A1h`. No media, no READ TOC/READ(10), no El Torito. See `docs/atapi-r4-packet-protocol.md`.
...
- [ ] CD-ROM boot — **not started; packet transport moved.** Round 4 runs three ATAPI packet opcodes on a detectable PACKET device, but there is still no CD-ROM media, no READ TOC/READ(10), no El Torito, no INT 13h CD-ROM emulation, and peripheral type remains `1Fh` rather than `05h`.
