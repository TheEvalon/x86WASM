...
- [ ] PCI configuration space — **partial:** `devices::PciConfig` Mechanism #1 (`0xCF8`/`0xCFC`) + host bridge `00:00.0` `8086:1237` + PIIX stubs `00:01.0` ISA `8086:7000` (PIRQRC `0x60`–`0x63` default `0x80` store/readback) / `00:01.1` IDE `8086:7010` (BMIBA I/O BAR config `0x20` store/readback, bit0 forced; IDETIM `0x40` word store/readback) / `00:01.2` USB `8086:7020` (UHCI BAR0 I/O BAR config `0x20` store/readback, 32-byte align, bit0 forced; LEGSUP `0xC0` dword store/readback) / `00:01.3` ACPI `8086:7113` (PMBASE I/O BAR config `0x40` mask `0xFFC0`|bit0 store/readback) on `MachineBus`; **not** USB host / ACPI PM I/O / SMI / tables / BMIDE port decode / BAR MMIO / bus mastering engine / caps/MSI/PCIe
- [ ] PIIX IDE — **partial:** `devices::IdePrimary` IDENTIFY (`0xEC`) + READ SECTORS (`0x20`) + WRITE SECTORS (`0x30`) + FLUSH CACHE (`0xE7`)/FLUSH CACHE EXT (`0xEA`) + READ NATIVE MAX ADDRESS (`0xF8`) + MEDIA LOCK/UNLOCK (`0xDE`/`0xDF`) + NOP (`0x00`) + IDLE (`0xE3`)/IDLE IMMEDIATE (`0xE1`)/STANDBY IMMEDIATE (`0xE0`)/STANDBY (`0xE2`)/SLEEP (`0xE6`) + CHECK POWER MODE (`0xE5`→sector_count=`0xFF`) + RECALIBRATE (`0x10`)/SEEK (`0x70`)/INITIALIZE DEVICE PARAMETERS (`0x91`) + EXECUTE DEVICE DIAGNOSTIC (`0x90`) + SET FEATURES (`0xEF`) success stubs PIO on ports `0x1F0`–`0x1F7`/`0x3F6` with backing image + DRQ/BSY/DRDY + IRQ14→`DualPic` when nIEN=0; PACKET (`0xA0`) + IDENTIFY PACKET (`0xA1`) + READ MULTIPLE (`0xC4`) + WRITE MULTIPLE (`0xC5`) + SET MULTIPLE MODE (`0xC6`) + SMART (`0xB0`) + READ DMA (`0xC8`) → ERR+ABRT on ATA master (no packet/multi/SMART/BM-DMA data; nIEN gates INTRQ); `devices::IdeSecondary` same PIO on `0x170`–`0x177`/`0x376` + IRQ15→`DualPic`; **not** PACKET media engine/SMART feature set/real BM-DMA (WRITE DMA/PRD)/slave/PCI BARs/SeaBIOS
