...
Status (2026-08-09, round-1 parallel integration `merge/m2-r1-parallel-16` on base `d95a4f5`): **in progress** — four parallel agents landed 16 bounded slices covering same-CPL **32-bit** protected mode, VGA plane/Graphics-Controller memory, IDE device-selection and 48-bit PIO, FDC READ ID / implied seek, and machine glue (COM IRQ routing, fw_cfg DMA, port `0x80`, POST probe). Protected mode is now default-32 capable at CPL 0 only: there is still **no privilege switching, call gate, LDT, TSS, or paging**. **SeaBIOS POST does not pass** — the measured first blocker is recorded below. M2 exit criteria remain open.
...
- [ ] Hard-disk boot â€” **partial:** `Machine::attach_ide_image` / `with_ide` + `Machine::load_mbr_to_7c00` copies primary IDE LBA0 (prefer) or floppy CHS `(0,0,1)` into phys `0x7C00`, requires `0x55AA` signature, sets `CS:IP = 0000:7C00` for synthetic MBR handoff tests; the IDE side now also answers ATA/ATAPI-6 §9.16.1 Device 0 only selection probes and 48-bit READ/WRITE SECTOR(S) EXT PIO, which is what firmware disk enumeration exercises first; **not** SeaBIOS INT 13h / partition walk / FreeDOS disk boot / CD-ROM / BM-DMA transfers driven by an ATA command
- [ ] CD-ROM boot

Measured SeaBIOS POST blockers (2026-08-09):

The new POST probe harness (`Machine::probe_post` / `emulator-cli --post-probe`) replaces guesswork with a measurement. Run against the pinned SeaBIOS `rel-1.16.3` `firmware/seabios/bios.bin` on this integration branch:

```text
post-probe: steps=2 stop=unsupported opcode 0x85 cs:ip=F000:E062 rip=0x000000000000E062 linear_pc=0x00000000000FE062 opcode_bytes=[0F 85 8E F9 31 D2 8E D2]
  post-codes=[] last=none
  com1="" debug=""
```

1. **Two-byte near `Jcc rel16/rel32` (`0F 80`–`0F 8F`) is not decoded.** The reset vector far-jumps to `F000:E05B`, which executes `CMP DWORD PTR CS:[0x9228], 0` and then reaches the `0F 85` (`JNZ`) at `F000:E062`. SeaBIOS cannot get past its first branch. This is an `x86-decode` / `x86-interpreter` gap and is the top item for round 2.
2. **Default-32 protected mode — closed by this round** (slices 1–4 above: `CS.D=1` execution, `SS.B=1` stacks, 386 IDT gates, `IRETD`). It was the previously assumed blocker; the measurement shows the CPU never reaches protected mode because of blocker 1.

Everything past blocker 1 is unmeasured: the probe stops at the first CPU-level failure, does not resume, and advances no time source, so a firmware path that polls the PIT or RTC would exhaust the step budget instead of progressing. Later rounds should re-run the probe after each CPU slice rather than predicting the next stop.
