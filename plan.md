...
- [ ] DMA — **partial:** `devices::Dma8237` dual 8237A addr/count/mode/mask + AT page regs on `MachineBus` + status TC bits (3:0) clear-on-read + `latch_tc` + `transfer_block` software helper (8-bit ch0–3, length `count+1`, phys `(page<<16)|addr`, Single+Increment+Read/Write mode subset, `mem_read`/`mem_write` callbacks; updates current addr/count + latches TC); **not** DREQ/DACK cycle timing / auto-init / decrement / demand|block|cascade|verify / ch4–7 word transfers / FDC|IDE|Machine PhysMem wiring / SeaBIOS floppy DMA
