...
- [ ] ATAPI — **partial: minimal packet + optional CD-ROM medium.** Minimal PACKET path: TUR / REQUEST SENSE / INQUIRY, type `1Fh`. CD-ROM capable path (`attach_atapi_cdrom`): type `05h`, READ CAPACITY / READ(10), ASC `3Ah` when empty. DEVICE RESET (`08h`) for PACKET devices. ATA disks still abort `PACKET`/`A1h`. No El Torito / INT 13h CD. See `docs/atapi-r4-packet-protocol.md`, `docs/atapi-r5-cdrom-medium.md`.
...
- [ ] CD-ROM boot — **still open.** Round 5 can attach a raw 2048-byte-block image and READ it on a type-`05h` device; El Torito / INT 13h CD emulation / boot handoff are not started.
