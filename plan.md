...
- [ ] PIIX IDE — **partial:** `devices::IdePrimary` IDENTIFY (`0xEC`) + READ SECTORS (`0x20`) + WRITE SECTORS (`0x30`) + FLUSH CACHE (`0xE7`) + NOP (`0x00`) + IDLE (`0xE3`)/IDLE IMMEDIATE (`0xE1`)/STANDBY IMMEDIATE (`0xE0`) + CHECK POWER MODE (`0xE5`→sector_count=`0xFF`) + EXECUTE DEVICE DIAGNOSTIC (`0x90`) + SET FEATURES (`0xEF`) success stubs PIO on ports `0x1F0`–`0x1F7`/`0x3F6` with backing image + DRQ/BSY/DRDY + IRQ14→`DualPic` when nIEN=0; PACKET (`0xA0`) + IDENTIFY PACKET (`0xA1`) + READ MULTIPLE (`0xC4`) + WRITE MULTIPLE (`0xC5`) → ERR+ABRT on ATA master (no packet/multi PIO; nIEN gates INTRQ); `devices::IdeSecondary` same PIO on `0x170`–`0x177`/`0x376` + IRQ15→`DualPic`; **not** PACKET media engine/DMA/slave/PCI BARs/SeaBIOS
