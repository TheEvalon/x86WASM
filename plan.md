...
- [ ] IDT — **partial:** IDTR is programmable via `LIDT`/`SIDT`; PE=0 uses the IVT, while PE=1 supports atomic same-CPL delivery through both 286 16-bit gates (`0x6`/`0x7`) and 386 32-bit gates (`0xE`/`0xF`) for architectural faults, software interrupts, NMI, and IRQ, including gate DPL rules and applicable word or doubleword error-code frames; frame width comes from the gate type and stack-pointer width from `SS.B`, and a 16-bit gate still requires `D=0` current and target code rather than truncating a 32-bit return `EIP`. Round 2 adds no gate mechanics, but two new sources now enter through these paths: `UD2`'s architecturally guaranteed `#UD`, and the `#GP(0)` every `RDMSR`/`WRMSR` address raises because no MSR is implemented. **Not:** task gates, gates in the LDT, privilege-stack switching, outer-level delivery, VM86 delivery, the IST mechanism, or nested #DF/triple-fault synthesis.
- [ ] LDT
- [ ] TSS
- [x] Exceptions — real-mode IVT delivery remains complete for #DE/#UD/#BR/#GP/#SS and #OF via INTO. PE=1 has bounded atomic delivery through same-CPL 16-bit *and* 386 32-bit IDT gates, including selector error codes for #GP/#NP/#SS (word through a 286 gate, doubleword through a 386 gate) and no error code for #DE/#BR/#UD. Round 2 adds `UD2` as a distinct architectural `#UD` (separable from a host decode gap), `#GP(0)` from every `RDMSR`/`WRMSR` address and from `RDMSR`/`WRMSR`/`INVD`/`WBINVD` outside CPL 0, `#UD` for Group 8 `/0`–`/3` and for the register form of `LSS`/`LFS`/`LGS`, and fault-ordering care in the `BT` family, where `CF` commits only after the read-modify-write cannot fault and the segment-limit check applies to the displaced bit-string byte rather than the bit base. A guest write into the PAM window **no longer produces a memory fault** at all — with WE clear it is dropped, as the chipset does. The full protected-mode taxonomy, nested #DF, and triple-fault behavior remain open.
...
- [ ] 32-bit paging — **partial (first real content), and connected to nothing:** `crates/x86-mmu/src/paging/` is a complete, tested 32-bit paging translation engine following SDM Vol. 3 Chapter 4 — `CR3` decode (Table 4-3), PDE/PTE decode (Tables 4-4…4-6), the §4.3 walk to a 4-KiB or 4-MiB page (Figures 4-2/4-3), §4.6.1 access rights including `CR0.WP`, the §4.7 Figure 4-12 `#PF` error code with `CR2` carrying the faulting offset, §4.8 Table 4-5 accessed/dirty flags, `CR4.PSE` large pages, PSE-36 (implemented, **off by default**), and a TLB with §4.10.2.2 entry contents, §4.10.2.4 global pages and the §4.10.4 `INVLPG` / `MOV CR3` / `MOV CR4` invalidation rules. Physical memory comes through a caller-supplied `PageTableMemory` trait, so the crate keeps its no-browser, no-`machine-pc` boundary. **Nothing calls it.** `CR0.PG` is not honored, `CR3`/`CR4` are not guest-writable, `#PF` is never delivered, the interpreter's memory path still treats a linear address as physical, and no firmware in the tree enables paging — so this is tested library behavior, not observed machine behavior, and the checkbox stays unticked. `CR0.PG = 0` returns `Unsupported(PagingDisabled)` rather than the identity mapping so a caller cannot skip the check by accident. **Documented deviation:** §4.8 read literally would set Accessed on entries a failing walk visited; this tree checks access rights first and commits A/D only for a translation that succeeds, per §4.10.2.3. `docs/mmu-r3-integration-surface.md` is the round-4 wiring contract. **Not:** PAE, 4-level or 5-level paging, `NX`, SMEP/SMAP, PAT/memory typing, or any interpreter integration.
...
- [ ] ATAPI — **partial: detection only.** An ATA device still rejects PACKET (`0xA0`) and IDENTIFY PACKET DEVICE (`0xA1`) with ABRT. Round 3 adds a *configurable* PACKET device on either channel (`IdePrimary::with_atapi_device` / `attach_atapi_device`, same on `IdeSecondary`) that firmware can **detect**: the `01h`/`01h`/`14h`/`EBh` signature (`ATAPI_SIGNATURE_*`) is written at power-on, on SRST and on EXECUTE DEVICE DIAGNOSTIC per §9.10/§9.11/§9.12 with Status reading `00h`; IDENTIFY DEVICE aborts with the full signature written (§8.15.5.2) and READ SECTOR(S) aborts with the signature in LBA Mid and High **only** (§8.34.5.2); IDENTIFY PACKET DEVICE (`0xA1`) returns a truthful 256-word block per §8.16 and Table 29, accepted regardless of DRDY, with nIEN gating INTRQ. **It reports peripheral device type `1Fh` "unknown or no device type", not `05h` CD-ROM** — claiming CD-ROM would advertise a command set that does not exist. **`PACKET` (`0xA0`) is still aborted on both device types**, and DEVICE RESET (`0x08`) is unimplemented despite being mandatory for a packet device. **Not:** any command packet set (no SFF-8020i/MMC), media, CD-ROM behavior, or ISO boot
...
- [ ] Initial VGA graphics — **partial (first real content):** `devices::VgaText` now models four 64 KiB maps plus the four Graphics Controller data latches. CPU-side address decode follows Sequencer Memory Mode and Map Mask (`plane_access`/`plane_offset`/`plane_write_mask`/`plane_size_bytes`): Chain 4 picks the map from A1:A0 and clears the low two address bits from the offset, Odd/Even sends even addresses to maps 0+2 and odd to maps 1+3 with A0 cleared, planar passes the offset through, and clearing Extended Memory wraps offsets inside 16 KiB per map. `gc_read_u8` loads all four latches and returns Read Map Select (or the Chain-4 address map) in read mode 0 or a Color Compare / Color Don't Care result in read mode 1; `gc_write_u8` implements write modes 0–3 with Set/Reset, Enable Set/Reset, Data Rotate + Function Select, Bit Mask, and Map Mask plane write enables. GC Miscellaneous bits 3:2 select the CPU display window (`A0000` 128 KB, `A0000` 64 KB, `B0000` 32 KB, `B8000` 32 KB) and bit1 Chain Odd/Even is a second odd/even source; Misc Output RAM Enable still gates every claim. Round 2 closes three gaps round 1 recorded. **The data path is now reachable from a guest**: `VgaText::mmio_read_u8`/`mmio_write_u8` are a single entry point covering RAM-Enable gating, window decode, plane addressing and the GC path, and `MachineBus` routes the fixed `0xA0000`–`0xBFFFF` aperture to them, so guest writes reach the write modes, the latches, and Map Mask. **Write mode 3 now applies Function Select**, following Abrash's *Graphics Programming Black Book* chapter 26 where the sources genuinely conflict with OSDev (see `docs/vga-r2-gc-datapath-fixes.md` and `docs/sources.md`); the reset default is replace, so the difference shows only with a non-zero Function Select left in the Data Rotate register. **Graphics Mode bit 4 now steers read-mode-0 map selection**: with Chain 4 inactive, host address bit A0 relative to the selected window base replaces bit 0 of Read Map Select, and since the mode-03h default sets the bit, a read returns the character map at even addresses and the attribute map at odd ones. See `docs/vga-plane-memory-model.md`. **Round 3 adds the read side and one display memory.** The two-backing-store artifact is retired: `VgaText::mem` is gone and every access, guest or host, resolves in the four maps. **Exactly two programmings render, and no others:** text mode 03h through the character generator (`docs/vga-r3-character-generator.md`) and chain-4 256-color mode 13h (`docs/vga-r3-graphics256-mode13h.md`), the latter recognized only when the whole signature is present — GC Miscellaneous Graphics/Alphanumeric, Graphics Mode `C256`, Sequencer Chain 4, ATC `ATGE` and `8BIT`. Anything else — every planar 16-color mode included — reports `VgaRenderMode::Unsupported` and `render_frame` returns `None` rather than rendering something the hardware would not show. Frame sizes are fixed (720×400 text, 320×200 mode 13h), **not** derived from CRTC timing. **Not:** a planar 16-color renderer (modes 0Dh/0Eh/10h/12h), VBE, any host display or canvas, timing-accurate raster or blink timing, a font at reset (there is no character ROM — nothing renders until software loads one); Shift Register Interleave has no effect; QEMU's alternative `addr >> 2` Chain-4 offset is not used; and CRTC byte/word (`Count by Two`) compensation is absent
- [ ] Initial VBE
