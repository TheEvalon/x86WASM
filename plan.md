...
- [ ] Initial VBE
- [ ] SeaBIOS integration — **partial:** `firmware_interface::prepare_bios_rom` + `Machine::load_bios_rom` / `with_bios_rom` dual-map a BIOS image at top-of-4 GiB and below-1 MiB alias (`0xF0000` for 64 KiB; last 128 KiB at `0xE0000` when larger); synthetic blob tests; `firmware/README.md` layout; `devices::FwCfg` traditional I/O `0x510`/`0x511` (signature `QEMU`, `FW_CFG_FILE_DIR` + one test file `opt/org.x86wasm/test`=`FWCF`) on `MachineBus`; **not** SeaBIOS POST / option ROM scan / fw_cfg DMA (`0x514`) / RAM-size·boot-order·ACPI·SMBIOS blobs / OVMF
