...
Status (2026-08-02, branch `feat/real-mode-int-iret`): **in progress** — real-mode foundation **audit tranche** landed (RM-D/E/F/G, REP, word strings, opsize32, #UD IVT, INS/OUTS, BCD, INTO/BOUND, ENTER nesting>0, 0F AF IMUL, decode-miss #UD + ModRM/stack/string/moffs #GP/#SS, address-size 0x67 ModRM/SIB + string ESI/EDI/ECX, **0x66 tranche-2**: ENTERD/LEAVE32, PUSHAD/POPAD, PUSHFD/POPFD, interruptible REP via `pending_irq` stub, **unreal segment limits** / sticky DS/ES/SS/FS/GS cache + moffs32); **not SeaBIOS-ready**. M2 exit criteria are not met.

Progress against implement list:

- [x] Partial real-mode foundation (software INT/IRET/INT3, PUSHF/POPF, far CALL/RETF/JMP, segment MOV/PUSH/POP, Jcc, XCHG, LOOP/JCXZ, Group 1/2/3 full F6/F7 /0-/7 TEST/NOT/NEG/MUL/IMUL/DIV/IDIV with #DE, Group 4/5 INC/DEC r/m FE/FF /0-/1, Group 5 CALL/JMP/PUSH r/m FF /2,/4,/6, Group 5 far CALL/JMP m16:16 FF /3,/5, string byte/word/dword ops MOVSB/W/D STOSB/W/D LODSB/W/D CMPSB/W/D SCASB/W/D (A4–A7/AA–AF) with REP/REPE/REPNE (CX=0 nop, CX loop, ZF early-exit, DF; 0x66 → dword), INS/OUTS string port I/O INSB/W/D OUTSB/W/D (6C–6F) with REP (DX port; ES:DI dest / DS:SI src with seg override on OUTS; DF steps; 0x66 → dword; `MachineBus` size-aware port_in/out), BCD adjust DAA/DAS/AAA/AAS/AAM/AAD (27/2F/37/3F/D4/D5; AAM base-0 #DE via IVT), LEA, CBW/CWD, flag ops, PUSH imm, SAHF/LAHF, INC/DEC r16, AND/OR ModRM 08-0B/20-23, AND/OR AL/AX imm 0C/0D/24/25, ADC/SBB ModRM 10-13/18-1B, ADC/SBB AL/AX imm 14/15/1C/1D, XOR ModRM 30-33, ADD/SUB ModRM byte 00/02/28/2A, SUB/XOR/CMP AL/AX imm 2C/2D/34/35/3C/3D, CMP ModRM byte 38/3A, ADD AX imm 05, related ALU ModRM forms, legacy high-byte ModR/M AL..BH via shared `gpr_u8`/`set_gpr_u8`, MOV C6/C7 r/m imm, MOV A0-A3 moffs16/moffs32 (0x67), TEST A8/A9 AL/AX imm (A9 opsize32 still open), RM-E stack/frame: PUSHA/POPA/PUSHAD/POPAD 60/61, ENTER/ENTERD nesting 0–31 / LEAVE/LEAVE32 C8/C9 (0x66 dword display), PUSHF/POPF/PUSHFD/POPFD 9C/9D, RET/RETF iw C2/CA, POP r/m16 8F /0, real-mode exception delivery via IVT for #DE/#UD/#GP/#SS (architectural decode-miss + ModRM/stack/string/moffs MemoryFault classify + cached segment-limit #GP/#SS), interruptible REP (`pending_irq` stub), RM-F LES/LDS/XLAT C4/C5/D7, RM-G IMUL imm 69/6B, IMUL 0F AF two-operand r16/r32↔r/m (0x66→32), INTO CE (#OF trap) / BOUND 62 (#BR fault), **0x66 opsize-32 tranche** for MOV r/m↔r / imm (89/8B/B8-BF/C7), PUSH/POP r32/imm (50-5F/68/6A), ALU ModRM+accum ADD/SUB/XOR/CMP/AND/OR/ADC/SBB + Group1 81/83, near JMP/CALL/RET, **tranche-2** ENTERD/LEAVE32 + PUSHAD/POPAD + PUSHFD/POPFD, **unreal limits**: sticky `load_real_mode_selector` for DS/ES/SS/FS/GS + `checked_linear_addr` on ModRM/moffs/stack/XLAT)
- [x] Complete real-mode foundation (**audit gaps landed**: RM-D/E/F/G, REP/REPE/REPNE + interruptible REP (`pending_irq` / `Bus::poll_external_irq`), word/dword strings, 0x66 opsize-32 tranche + tranche-2 ENTERD/PUSHAD/PUSHFD, #UD IVT, INS/OUTS, BCD, INTO/BOUND, ENTER nesting>0, 0F AF IMUL, decode-miss architectural #UD + stack/ModRM/string/moffs #GP/#SS classify, address-size 0x67 ModRM/SIB EA + string ESI/EDI/ECX + moffs32, unreal segment limits / sticky data-seg cache) — **not SeaBIOS-ready**; remaining foundation gaps: string ops still skip cached limit checks (bus MemoryFault classify only); CS instruction-fetch limit checks; remaining 0x66: INC/DEC r32 (40–4F), Group5 INC/DEC/PUSH r/m32 + far m16:32 (FF), TEST EAX imm32 (A9), CWDE/CDQ (98/99), XCHG EAX,r32 (90–97), LES/LDS r32 (C4/C5), BOUND r32 (62), far CALL/JMP/RETF ptr16:32; JECXZ/LOOP(E)CX + XLAT EBX under asize32; MOVSQ/STOSQ/… qword strings; per-instruction (non-REP) external IRQ poll; full 8259 PIC; fetch MemoryFault classify; LGDT/PM loaders to *set* unreal limits (tests poke `SegmentReg.limit` directly).
- [ ] Protected mode
...
- [ ] LDT
- [ ] TSS
- [x] Exceptions (real-mode IVT fault delivery for #DE/#UD/#BR/#GP/#SS; #OF trap via INTO; software INT/INT3/INTO) — protected-mode / full fault taxonomy still open
- [ ] Interrupts (hardware / PIC-driven) — stub: `CpuState::pending_irq` / `request_interrupt` + REP iteration poll when IF=1; no 8259 yet
- [ ] 32-bit paging
- [ ] 8259 PIC
- [ ] 8254 PIT
- [ ] RTC
