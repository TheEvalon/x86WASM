...
- [ ] PCI configuration space — **partial:** `devices::PciConfig` Mechanism #1 (`0xCF8`/`0xCFC`) + host bridge `00:00.0` `8086:1237` (Command sticky IO|MEM|BusMaster; Status CapList=0/FastB2B/DevSel=medium `0x0280` + RW1C) + PIIX stubs `00:01.0` ISA `8086:7000` (Command sticky IO|MEM|BusMaster — `PCI_PIIX_ISA_COMMAND_MASK`=`0x0007` mirrors host bridge; PIRQRC `0x60`–`0x63` default `0x80` store/readback; ELCR `0x4D0`/`0x4D1` store/readback reset `0x00`, SeaBIOS/PIIX edge/level — not wired to DualPic LTIM) / `00:01.1` IDE `8086:7010` (Command sticky IO|BusMaster; Status CapList=0/FastB2B/DevSel=medium `PCI_PIIX_IDE_STATUS_STUB`=`0x0280` + RW1C; BMIBA I/O BAR config `0x20` store/readback, bit0 forced; IDETIM `0x40` word store/readback) / `00:01.2` USB `8086:7020` (UHCI BAR0 I/O BAR config `0x20` store/readback, 32-byte align, bit0 forced; LEGSUP `0xC0` dword store/readback) / `00:01.3` ACPI `8086:7113` (PMBASE I/O BAR config `0x40` mask `0xFFC0`|bit0 store/readback) on `MachineBus`; **not** USB host / ACPI PM I/O / SMI / tables / BMIDE port decode / BAR MMIO / bus mastering engine / Command decode side effects / Status error signaling / ISA Status / USB·ACPI Command·Status / caps/MSI/PCIe / ELCR→PIC LTIM
