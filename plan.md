...
- [ ] Interrupts (hardware / PIC-driven) — **partial:** `CpuState::pending_irq` / `request_interrupt` + per-instruction + REP poll when IF=1; `CpuState::pending_nmi` / `request_nmi` + `Machine::inject_nmi` (CMOS-gated `#NMI` vector 2, IF-independent); `MachineBus::poll_external_irq` syncs PIT ch0 OUT→IRQ0 + 8042 OBFâˆ§INT1→IRQ1 + FDC IRQ6 + CMOS IRQF→IRQ8 + 8042 AUX OBFâˆ§INT12→IRQ12 + primary IDE→IRQ14 + secondary IDE→IRQ15 then `DualPic::poll_irq`
- [ ] 32-bit paging
