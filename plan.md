...
Status (2026-08-11, round-15 boot-guest lane `slice/r15-boot-guest` on base `f7f7a60`): **in progress -- R15 boot-guest lane.** POST-with-media `F000:C897` classify as `wait-irq-yield` (idle/busy + CF9; not POST complete); host INT19-order handoff → `guest-halted-at-boot-sector`; FreeDOS v8 FAT12 `KERNEL.SYS` locate → `kernel-name-located-missing-load`; INT13 AH=02h CHS+BDA `0040:0074` FreeDOS-path deepen; Linux bzImage setup entry + El Torito payload peek (`no-emul-bzimage`). See `docs/boot-r15-*.md`, `docs/storage-r15-int13-read-chs.md`. **Not** FreeDOS prompt / Linux shell; SeaBIOS INT19 still unmeasured at C897.
