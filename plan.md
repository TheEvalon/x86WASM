...
Status (2026-08-11, round-15 usb-timer lane `slice/r15-usb-timer` on base `f7f7a60`): **in progress -- R15 usb-timer lane.** UHCI PORTSC reset/connect deepen; UHCI TD short-packet/Stall honesty (UHCI 1.1); LAPIC LDR/DFR sticky stub (**no CPUID.APIC**); HPET main-counter freerun via PIT tick (12×) keeping LEG_RT clear. See `docs/uhci-r15-*.md`, `docs/lapic-r15-ldr-dfr.md`, `docs/hpet-r15-main-counter.md`.
...
Status (2026-08-11, round-15 boot-guest lane `slice/r15-boot-guest` on base `f7f7a60`): **in progress -- R15 boot-guest lane.** POST-with-media `F000:C897` classify as `wait-irq-yield` (idle/CF9 signal; not 7C00); host INT19-order handoff measure reaches `guest-halted-at-boot-sector`; FreeDOS v8 FAT12 `KERNEL.SYS` locate → `kernel-name-located-missing-load`; Linux bzImage setup entry + El Torito payload peek. See `docs/post-r15-c897-with-media.md`, `docs/boot-r15-*.md`. **Not** FreeDOS prompt / Linux shell; SeaBIOS INT19 still unmeasured; INT13 AH=02/42 left to storage lane.
...
Status (2026-08-11, round-15 display-fw lane `slice/r15-display-fw` on base `f7f7a60`): **in progress -- R15 display-fw lane.** Host INT 10h AH=05h select active page (BDA+CRTC Start Address), AH=13h write-string deepen (scroll + AL=01/03 cursor), AH=0Eh teletype scroll path coherent with R14 AH=06h + CR/LF/BS/BEL + active-page BH, AH=09h/0Ah active-page polish, VBE AX=4F03h get-mode without LFB bit; **no guest LFB**. See `docs/vga-r15-*.md`. **Not** SeaVGABIOS VBE body; PhysBasePtr stays zero.
