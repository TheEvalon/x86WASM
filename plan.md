...
Status (2026-08-11, round-15 display-fw lane `slice/r15-display-fw` on base `f7f7a60`): **in progress -- R15 display-fw lane.** Host INT 10h AH=05h select active page (BDA+CRTC Start Address), AH=0Eh teletype CR/LF/BS/BEL honesty, AH=09h/0Ah active-page count/attr polish, VBE AX=4F03h get-mode + 4F0Ah PMI fail without LFB bit; **no guest LFB**. See `docs/vga-r15-*.md`. **Not** SeaVGABIOS VBE body; PhysBasePtr stays zero.
