...
- [ ] Interrupts (hardware / PIC-driven) — **partial:** `CpuState::pending_irq` / `request_interrupt` + per-instruction + REP poll when IF=1; `MachineBus::poll_external_irq` syncs PIT ch0 OUT→IRQ0 + 8042 OBF∧INT1→IRQ1 then `DualPic::poll_irq`; **not** CMOS IRQ8 device wiring yet
- [ ] 32-bit paging
