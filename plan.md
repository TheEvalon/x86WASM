...
- [ ] DMA — **partial:** `devices::Dma8237` dual 8237A addr/count/mode/mask + AT page regs on `MachineBus` + status TC bits (3:0) clear-on-read + `latch_tc` device/test API (no memory move); **not** DREQ/DACK transfer engine / hardware TC from transfers / device integration / SeaBIOS floppy DMA
